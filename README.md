...
起動後、`http://localhost:8080/api/categories` 等でAPIの疎通を確認できます。初回起動時、`schema.sql`/`data.sql` によりテーブル作成と初期データ（カテゴリ7件、定期支出テンプレート2件、当月・先月分の収支記録サンプル20件）の投入が自動で行われ、起動直後から一覧・カテゴリ管理・定期支出・月次集計の全画面にデータが表示された状態で試せます。サンプルデータの日付は起動時点の当月・先月を基準にするため、いつクローンして起動しても最新の当月データとして表示されます。
