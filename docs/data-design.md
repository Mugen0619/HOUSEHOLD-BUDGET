...
| last_generated_month | DATE | NULL可（月初日で保持） | 自動生成バッチが「どの年月分まで生成済みか」を記録する内部状態（5章参照）。NULL＝未生成 |
...
- `last_generated_month` は自動生成バッチのみが更新する内部状態で、API（4.3・5章）には含めない。生成済み収支記録の `date` は通常の収支記録として自由に編集できるため、「生成済みか」の判定にこの列を使わないと、日付編集で二重生成・欠落が起こり得る
...
- 各テンプレートについて、「どの年月分まで生成済みか」を `recurring_transactions.last_generated_month`（月初日で保持、未生成ならNULL）で管理する。生成済み収支記録の `date` は編集で自由に変更できる（5.1補足参照）ため、`transactions` テーブルの現在値からは判定しない
- 未生成の月（`last_generated_month` の翌月〜当月、テンプレート未生成の場合は作成月〜当月）を古い順にたどり、その月の実行日（5.2・5.3参照）を過ぎているものだけ収支記録を生成する（`date` = その月の実行日、`source = 'RECURRING'`）。生成するたびに `last_generated_month` をその月に更新する
- 実行日がまだ来ていない月に到達した時点でその月以降の生成を打ち切る（それより先の月はさらに未来のため）

### 5.2 常時起動していない環境への対応（キャッチアップ方式）

このアプリはDocker/バックエンド/フロントエンドを手動で起動するローカル運用（要件定義書7章：公開環境への常時デプロイは対象外）のため、「毎日決まった時刻に必ず実行される」ことは前提にできない（例：実行日を過ぎてからアプリを起動するケースや、数ヶ月アプリを起動しないケースがある）。

そのため、5.1のジョブは「その時刻ちょうどに動く」ことを目的とせず、**「アプリが起動している間に、まだ生成されていない月があればすべて遡って生成する」というキャッチアップ方式**とする。これにより、アプリ起動時にも同じジョブ（または起動時に1回限りの同等チェック）を実行しておけば、実行日から日にちが経っていても、複数月にわたってアプリを起動していなかったとしても、次にアプリを起動したタイミングで未生成分がすべて確実に生成される。
